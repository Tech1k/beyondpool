...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        print 'beyondpool (version %s)' % (p2pool.__version__,)
        print
        
        @defer.inlineCallbacks
        def connect_p2p():
            # connect to beyondcoind over beyondcoin-p2p
            print '''Testing beyondcoind P2P connection to '%s:%s'...''' % (args.beyondcoind_address, args.beyondcoind_p2p_port)
            factory = bitcoin_p2p.ClientFactory(net.PARENT)
            reactor.connectTCP(args.beyondcoind_address, args.beyondcoind_p2p_port, factory)
            def long():
                print '''    ...taking a while. Common reasons for this include all of beyondcoind's connection slots being used...'''
        ...
        if args.testnet: # establish p2p connection first if testnet so beyondcoind can work without connections
            factory = yield connect_p2p()
        
        # connect to beyondcoind over JSON-RPC and do initial getmemorypool
        url = '%s://%s:%i/' % ('https' if args.beyondcoind_rpc_ssl else 'http', args.beyondcoind_address, args.beyondcoind_rpc_port)
        print '''Testing beyondcoind RPC connection to '%s' with username '%s'...''' % (url, args.beyondcoind_rpc_username)
        bitcoind = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.beyondcoind_rpc_username + ':' + args.beyondcoind_rpc_password)), timeout=30)
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                ...
                if not res['isvalid'] or not res['ismine']:
                    print '    Cached address is either invalid or not controlled by local beyondcoind!'
                    address = None
            
            if address is None:
                print '    Getting payout address from beyondcoind...'
                address = yield deferral.retry('Error getting payout address from beyondcoind:', 5)(lambda: bitcoind.rpc_getaccountaddress('beyondpool'))()
            ...
            for i in range(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from beyondcoind:', 5)(lambda: bitcoind.rpc_getnewaddress('beyondpool'))()
        ...
        print 'Joining beyondpool network using port %i...' % (args.beyondpool_port,)
        ...
        connect_addrs = set()
        for addr_df in map(parse, args.beyondpool_nodes):
            try:
        ...
        node.p2p_node = p2pool_node.P2PNode(node,
            port=args.beyondpool_port,
            max_incoming_conns=args.beyondpool_conns,
            addr_store=addrs,
            connect_addrs=connect_addrs,
            desired_outgoing_conns=args.beyondpool_outgoing_conns,
            advertise_ip=args.advertise_ip,
            external_ip=args.beyondpool_external_ip,
        )
        node.p2p_node.start()
                ...
                while True:
                    try:
                        ...
                        if is_lan:
                            ...
                            yield pm._upnp.add_port_mapping(lan_ip, args.beyondpool_port, args.beyondpool_port, 'beyondpool', 'TCP')
        ...
        if args.donation_percentage > 2.6:
            print '''Donating %.1f%% of work towards BeyondPool's development. Thanks for the tip!''' % (args.donation_percentage,)
        elif args.donation_percentage < 2.4:
            print '''Donating %.1f%% of work towards BeyondPool's development. Please donate to encourage further development of BeyondPool!''' % (args.donation_percentage,)
        else:
            print '''Donating %.1f%% of work towards BeyondPool's development. Thank you!''' % (args.donation_percentage,)
        ...
        if args.irc_announce:
            ...
            class IRCClient(irc.IRCClient):
                nickname = 'beyondpool%02i' % (random.randrange(100),)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    this_str = 'BeyondPool: %i shares in chain (%i verified/%i total) Peers: %i (%i incoming)' % (
                        height,
...
def run():
    ...
    parser = fixargparse.FixedArgumentParser(description='beyondpool (version %s)' % (p2pool.__version__,), fromfile_prefix_chars='@')
    parser.add_argument('--version', action='version', version=p2pool.__version__)
    parser.add_argument('--net',
        help='use specified network (default: beyondcoin)',
        action='store', choices=sorted(realnets), default='beyondcoin', dest='net_name')
    ...
    parser.add_argument('-a', '--address',
        help='generate payouts to this address (default: <address requested from beyondcoind>), or (dynamic)',
        type=str, action='store', default=None, dest='address')
    parser.add_argument('-i', '--numaddresses',
        help='number of beyondcoin auto-generated addresses to maintain for getwork dynamic address allocation',
    ...
    parser.add_argument('--datadir',
        help='store data in this directory (default: <directory run_beyondpool.py is in>/data)',
    ...
    parser.add_argument('--coinbtext',
        ...
        type=str, action='append', default=['/BeyondPool/'], dest='coinb_texts')
    parser.add_argument('--give-author', metavar='DONATION_PERCENTAGE',
        help='donate this percentage of work towards the development of beyondpool (default: 2.5)',
    ...
    parser.add_argument('--irc-announce',
        help='announce any blocks found on irc://irc.freenode.net/#beyondpool',
        action='store_true', default=False, dest='irc_announce')
    
    p2pool_group = parser.add_argument_group('beyondpool interface')
    p2pool_group.add_argument('--beyondpool-port', metavar='PORT',
        ...
        type=int, action='store', default=None, dest='beyondpool_port')
    p2pool_group.add_argument('-n', '--beyondpool-node', metavar='ADDR[:PORT]',
        help='connect to existing beyondpool node at ADDR listening on port PORT (defaults to default beyondpool P2P port) in addition to builtin addresses',
        type=str, action='append', default=[], dest='beyondpool_nodes')
    parser.add_argument('--disable-upnp',
        help='''don't attempt to use UPnP to forward beyondpool's P2P port from the Internet to this computer''',
    ...
    p2pool_group.add_argument('--max-conns', metavar='CONNS',
        ...
        type=int, action='store', default=40, dest='beyondpool_conns')
    p2pool_group.add_argument('--outgoing-conns', metavar='CONNS',
        help='outgoing connections (default: 6)',
        type=int, action='store', default=6, dest='beyondpool_outgoing_conns')
    p2pool_group.add_argument('--external-ip', metavar='ADDR[:PORT]',
        ...
        type=str, action='store', default=None, dest='beyondpool_external_ip')
    ...
    worker_group.add_argument('-f', '--fee', metavar='FEE_PERCENTAGE',
        help='''charge workers mining to their own beyondcoin address (by setting their miner's username to a beyondcoin address) this percentage fee to mine on your beyondpool instance. Amount displayed at http://127.0.0.1:WORKER_PORT/fee (default: 0)''',
        type=float, action='store', default=0, dest='worker_fee')
    
    bitcoind_group = parser.add_argument_group('beyondcoind interface')
    bitcoind_group.add_argument('--beyondcoind-config-path', metavar='BEYONDCOIND_CONFIG_PATH',
        help='custom configuration file path (when beyondcoind -conf option used)',
        type=str, action='store', default=None, dest='beyondcoind_config_path')
    bitcoind_group.add_argument('--beyondcoind-address', metavar='BEYONDCOIND_ADDRESS',
        help='connect to this address (default: 127.0.0.1)',
        type=str, action='store', default='127.0.0.1', dest='beyondcoind_address')
    bitcoind_group.add_argument('--beyondcoind-rpc-port', metavar='BEYONDCOIND_RPC_PORT',
        help='''connect to JSON-RPC interface at this port (default: %s <read from beyondcoin.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.RPC_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='beyondcoind_rpc_port')
    bitcoind_group.add_argument('--beyondcoind-rpc-ssl',
        help='connect to JSON-RPC interface using SSL',
        action='store_true', default=False, dest='beyondcoind_rpc_ssl')
    bitcoind_group.add_argument('--beyondcoind-p2p-port', metavar='BEYONDCOIND_P2P_PORT',
        help='''connect to P2P interface at this port (default: %s <read from beyondcoin.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.P2P_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='beyondcoind_p2p_port')
    bitcoind_group.add_argument(metavar='BEYONDCOIND_RPCUSERPASS',
        help='beyondcoind RPC interface username, then password, space-separated (only one being provided will cause the username to default to being empty, and none will cause BeyondPool to read them from beyondcoin.conf)',
        type=str, action='store', default=[], nargs='*', dest='beyondcoind_rpc_userpass')
    ...
    if len(args.beyondcoind_rpc_userpass) > 2:
        parser.error('a maximum of two arguments are allowed')
    args.beyondcoind_rpc_username, args.beyondcoind_rpc_password = ([None, None] + args.beyondcoind_rpc_userpass)[-2:]
    
    if args.beyondcoind_rpc_password is None:
        conf_path = args.beyondcoind_config_path or net.PARENT.CONF_FILE_FUNC()
        if not os.path.exists(conf_path):
            parser.error('''Beyondcoin configuration file not found. Manually enter your RPC password.\r\n'''
                '''If you actually haven't created a configuration file, you should create one at %s with the text:\r\n'''
                '''\r\n'''
                '''server=1\r\n'''
                '''rpcpassword=%x\r\n'''
                '''\r\n'''
                '''Keep that password secret! After creating the file, restart Beyondcoin.''' % (conf_path, random.randrange(2**128)))
        ...
        for conf_name, var_name, var_type in [
            ('rpcuser', 'beyondcoind_rpc_username', str),
            ('rpcpassword', 'beyondcoind_rpc_password', str),
            ('rpcport', 'beyondcoind_rpc_port', int),
            ('port', 'beyondcoind_p2p_port', int),
        ]:
        ...
        if 'rpcssl' in contents and contents['rpcssl'] != '0':
            args.beyondcoind_rpc_ssl = True
        if args.beyondcoind_rpc_password is None:
            parser.error('''Beyondcoin configuration file didn't contain an rpcpassword= line! Add one!''')
    
    if args.beyondcoind_rpc_username is None:
        args.beyondcoind_rpc_username = ''
    
    if args.beyondcoind_rpc_port is None:
        args.beyondcoind_rpc_port = net.PARENT.RPC_PORT
    
    if args.beyondcoind_p2p_port is None:
        args.beyondcoind_p2p_port = net.PARENT.P2P_PORT
    
    if args.beyondpool_port is None:
        args.beyondpool_port = net.P2P_PORT
    
    if args.beyondpool_outgoing_conns > 10:
