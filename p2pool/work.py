...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def freshen_addresses(self, c):
        ...
        self.address = yield deferral.retry('Error getting a dynamic address from beyondcoind:', 5)(lambda: self.bitcoind.rpc_getnewaddress('beyondpool'))()
    ...
    def preprocess_request(self, user):
        if (self.node.p2p_node is None or len(self.node.p2p_node.peers) == 0) and self.node.net.PERSIST:
            raise jsonrpc.Error_for_code(-12345)(u'beyondpool is not connected to any peers')
        if time.time() > self.current_work.value['last_update'] + 60:
            raise jsonrpc.Error_for_code(-12345)(u'lost contact with beyondcoind')
    ...
    def get_work(self, pubkey_hash, desired_share_target, desired_pseudoshare_target):
        ...
        if self.node.best_share_var.value is None and self.node.net.PERSIST:
            raise jsonrpc.Error_for_code(-12345)(u'beyondpool is downloading shares')
            ...
            try:
                if pow_hash <= header['bits'].target or p2pool.DEBUG:
                    ...
                    if pow_hash <= header['bits'].target:
                        print
                        print 'GOT BLOCK FROM MINER! Passing to beyondcoind! %s%064x' % (self.node.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, header_hash)
                        print
            except:
