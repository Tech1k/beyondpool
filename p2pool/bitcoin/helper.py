...
@deferral.retry('Error while checking Beyondcoin connection:', 1)
@defer.inlineCallbacks
def check(bitcoind, net):
    if not (yield net.PARENT.RPC_CHECK(bitcoind)):
        print >>sys.stderr, "    Check failed! Make sure that you're connected to the right beyondcoind with --beyondcoind-rpc-port!"
...
@deferral.retry('Error getting work from beyondcoind:', 3)
@defer.inlineCallbacks
def getwork(bitcoind, use_getblocktemplate=False):
    ...
    try:
        ...
        try:
            ...
            print >>sys.stderr, 'Error: Beyondcoin version too old! Upgrade to v0.5 or newer!'
            raise deferral.RetrySilentlyException()
...
def submit_block_p2p(block, factory, net):
    if factory.conn.value is None:
        print >>sys.stderr, 'No beyondcoind connection when block submittal attempted! %s%064x' % (net.PARENT.BLOCK_EXPLORER_URL_PREFIX, bitcoin_data.hash256(bitcoin_data.block_header_type.pack(block['header'])))
        raise deferral.RetrySilentlyException()
